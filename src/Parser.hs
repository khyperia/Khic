...
pint :: Parser Int
pint = (foldl' (\a i -> a * 10 + digitToInt i) 0 <$> many1 digit) <* spaces
...
parseType = foldl FunctionType <$> basic <*> many (parenCommas parseType)
...
parseExpr = buildExpressionParser table primary
  where table = [[Postfix (flip MethodCall <$> parenCommas parseExpr)]
                ,[Prefix (Cast <$> (tokenSymbol "@" *> parseType))]
                ,[prefix "-" Negation]
...
parseStatement = parseIf <|> parseWhile <|> parseReturn <|> (ExprStatement <$> parseExpr)
